...
> **Internal package — not published to npm.** This is a build-time-only member of the
> kotlin-docs-kit monorepo. Its `dist/` output is vendored into the engine packages at build
> time: bundled into [`@ktdocs/docusaurus-preset`](../docusaurus) and rsynced into
> [`kotlin-docs-hugo`](../hugo). You never install it directly — pick an engine.
...
The BEM `kt-*` class structure is the contract between the two engine packages. See [`docs/contracts/components.md`](../../docs/contracts/components.md) (issue #5) for the full reference, and the [preview specimens](../../docs/contracts/preview/) for live HTML examples.
